...
export default function CalculeInPeriod({
  ouvertPourPeriod,
  setOuvertPourPeriod,
  sessionDates,
  techerSessionId,
  ...
  const [calculateFrom, setCalculateFrom] = useState(
    sessionDates[0].substring(0, 10)
  );
  const [calculateTo, setCalculateTo] = useState(
    sessionDates[1].substring(0, 10)
  );
      ...
      <div className="resultsY">
        ...
        <div className="line-in-calculate-period">
          <div className="from-to-Y">
            ...
            <input
              type="date"
              onChange={(e) => {
                setCalculateFrom(e.target.value);
              }}
        ...
        {errorDate && (
          <div className="lineY">
            ...
            >
              Vérifier Votre Intervale De Date !
            </p>
          </div>
        )}
        ...
        <Spin tip="Loading..." spinning={fetchHoursInPeriode}>
          <div
            className={`resultsY-in-Period-close ${
              afficheCalcule ? "openpen" : ""
            }`}
          >
            <div className="lineY">
              <p className="keyY">Heures Supplémentaires Total:</p>
              <p className="valueY">
                {result ? result.totalHoursForAllMonths : 0}
              </p>
            ...
            {result && result.combinedResult.Cours && (
              <div className="lineY">
                <p className="keyY">Total Cours:</p>
                <p className="valueY">
                  {result && result.combinedResult.Cours
                    ? result.combinedResult.Cours
                    : 0}
                </p>
                <p className="unitéY">Heures</p>
              </div>
            )}
            {result && result.combinedResult.Tp && (
              <div className="lineY">
                <p className="keyY">Total Tps:</p>
                <p className="valueY">
                  {result && result.combinedResult.Tp
                    ? result.combinedResult.Tp
                    : 0}
                </p>
                <p className="unitéY">Heures</p>
              </div>
            )}
            {result && result.combinedResult.Td && (
              <div className="lineY">
            ...
            <div
              className="lineY"
              style={{ borderTop: "solid 1px black", paddingTop: "8px" }}
            >
